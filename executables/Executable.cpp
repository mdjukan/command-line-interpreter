...
void Executable::set_in(std::istream* in) {
    m_in      = in;
    m_owns_in = false;
}

bool Executable::in_redirected() { return m_in != &std::cin; }

void Executable::set_out(std::ostream* out) {
    m_out      = out;
    m_owns_out = false;
}

bool Executable::out_redirected() { return m_out != &std::cout; }

bool Executable::writes_to_file() { return dynamic_cast<std::fstream*>(m_out) != nullptr; }
...
void Executable::redirect_in(const std::string& file_path) {
    ...
    if (in->is_open()) {
        ...
        throw new RuntimeError("Cannot open file " + file_path);
    }
}
